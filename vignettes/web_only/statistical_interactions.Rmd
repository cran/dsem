...
vignette: >
  %\VignetteIndexEntry{Statistical interactions}
...
```{r make_sem, echo=TRUE, message=FALSE}
#
sem = "
...
```{r fit_dsem, echo=TRUE, message=FALSE}
fit = dsem(
  tsdata = Z,
  sem = sem,
...
```{r plot_response, echo=TRUE, message=FALSE}
T_j = seq( 5, 19, by = 0.1 )
X_j = T_j - 10
...
```{r plot_states, echo=TRUE, message=FALSE}
#
...
```{r plot_graph, echo=TRUE, message=FALSE}
library(igraph)
library(ggraph)
